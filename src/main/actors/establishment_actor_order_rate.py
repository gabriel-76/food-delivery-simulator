...
class EstablishmentActorOrderRate(EstablishmentActor):
    ...
    def time_to_prepare_order(self, order):
        time_to_prepare = round(random.expovariate(1 / self.establishment.order_production_time_rate))
        return time_to_prepare

    def time_estimate_to_prepare_order(self, order):
        return self.establishment.overloaded_until + self.time_to_prepare_order(order)
